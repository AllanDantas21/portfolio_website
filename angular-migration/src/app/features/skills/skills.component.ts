        ...
        <div class="relative">
          ...
          <div class="space-y-12 md:space-y-16 relative">
            @for (exp of experiences; track exp.id; let i = $index) {
              <div class="flex flex-col md:flex-row items-stretch justify-between w-full md:even:flex-row-reverse relative">
                  ...
                  <div>
                    ...
                    <div class="flex flex-wrap gap-1.5">
                      @for (tech of exp.technologies; track tech) {
                        <span class="px-2.5 py-1 text-xs rounded border border-gray-300 dark:border-gray-700 bg-gray-100/40 dark:bg-gray-900/60 text-gray-700 dark:text-gray-300 font-medium">
                          {{ tech }}
                        </span>
                      }
                    </div>
      ...
      <section [@fadeInUp] class="mb-16">
          ...
          @for (tab of tabs; track tab.id) {
            <button (click)="activeTab = tab.id"
                    [ngClass]="{
                      ...
                      'border-transparent text-gray-600 dark:border-gray-400 hover:text-gray-800 dark:hover:text-gray-200': activeTab !== tab.id
                    }"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
          @for (skill of getActiveSkills(); track skill.name) {
            <div (mouseenter)="hoveredCard = skill.name"
              ...
              <div class="w-12 h-12 flex items-center justify-center">
                @if (getSkillIconSlug(skill.name)) {
                  <img [src]="'https://cdn.simpleicons.org/' + getSkillIconSlug(skill.name) + '/' + (skill.name === 'Next.js' ? 'currentColor' : skill.color.replace('#', ''))" 
      ...
      <section [@fadeInUp] class="mb-16">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          @for (lang of languages; track lang.language) {
            <div (mouseenter)="hoveredLang = lang.language"
      ...
      <section [@fadeInUp] class="mb-12">
        ...
        <div class="bg-white dark:bg-[#1a2234] border border-gray-300 dark:border-gray-800/80 rounded-xl p-8 shadow-sm hover:shadow-md transition-all duration-300">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="space-y-4">
              @for (skill of softSkills.slice(0, 3); track skill) {
                <div [ngClass]="getSoftSkillClass(skill)"
            ...
            <div class="space-y-4">
              @for (skill of softSkills.slice(3, 6); track skill) {
                <div [ngClass]="getSoftSkillClass(skill)"
                     class="px-4 py-2.5 rounded-lg text-sm font-bold tracking-wide border shadow-sm transition-all duration-200 hover:scale-[1.02] select-none text-center md:text-left">
                  {{ 'skills.softSkillsList.' + skill | translate }}
                </div>
