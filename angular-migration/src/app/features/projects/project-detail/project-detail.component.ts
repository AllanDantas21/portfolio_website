    ...
    <app-container>
      @if (detail$ | async; as detail) {
        <div [@fadeInUp] class="w-full">
          ...
          @if (detail.image1) {
            <div class="mb-6 rounded-xl overflow-hidden shadow border border-gray-200/40 dark:border-gray-800/40 bg-gray-50 dark:bg-gray-900/50">
              <img
                [src]="detail.image1"
                [alt]="detail.title"
          ...
          @if (detail.description3) {
            <p class="text-gray-700 dark:text-gray-300 leading-relaxed text-sm sm:text-base mb-6 text-justify">
              {{ detail.description3 | translate }}
            </p>
          }
          @if (detail.description4) {
            <p class="text-gray-700 dark:text-gray-300 leading-relaxed text-sm sm:text-base mb-6 text-justify">
              {{ detail.description4 | translate }}
            </p>
          }
          @if (detail.description5) {
            <p class="text-gray-700 dark:text-gray-300 leading-relaxed text-sm sm:text-base mb-6 text-justify">
